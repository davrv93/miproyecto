app.controller("PersonaNaturalCtrl", function($scope, $mdDialog, msg, API_PERSONA, $rootScope) {
    ...
    $scope.onList = function(query){
        ...
        API_PERSONA.PersonaNatural.get($scope.params || $scope.qryPag).$promise.then(function (r) {       
                console.log(r);
                $scope.dataPersonaNatural = r;
                ...
                $scope.toggle = function (lista, lst)
                    ...
                    {
                        return ($scope.selectedItems.length !== 0 && $scope.selectedItems.length !== $scope.dataPersonaNatural.length);
                    };
                    $scope.isChecked = function()
                    {
                        return $scope.selectedItems.length === $scope.dataPersonaNatural.length;
                    };
                    
                    $scope.toggleAll = function()
                    {
                        if ($scope.selectedItems.length === $scope.dataPersonaNatural.length)
                        {
                            $scope.selectedItems = [];
                        ...
                        {
                            $scope.items = [];
                            angular.forEach($scope.dataPersonaNatural.results, function(value, key) {
    ...
    $scope.showColumn = [
        ...
        { index: 1, state: true, view: 'table_head.nro',        type: 'contador'},
        { index: 2, state: true, view: 'table_head.nombre',   type: 'nombre'},
        { index: 3, state: true, view: 'table_head.estado',   type: 'estado'},
        { index: 4, state: true,  view: 'table_head.opciones', type: 'Acciones'}
    ...
    $scope.new_edit = function(event, obj){
        ...
        $mdDialog.show({
              ...
              controller: "formPersonaNatural",
              templateUrl: 'upeu_erp_apps/config/views/deporte/form.html',
    ...
    $scope.changeState = function (state) {
        ...
        API_PERSONA.PersonaNatural.update({ id: 0 } , data).$promise.then(function(success) {
            $scope.refresh();
        },function(error) {
    ...
    $scope.destroyMany = function (obj) {
        ...
        API_PERSONA.PersonaNatural.destroy({ microrecurso: 'destroy',bulk_id : $scope.selectedItems}).$promise.then(function(success) {
           $scope.refresh();
            msg.yesDelete();
            $scope.selectedItems = [];
        },function(error) {
...
app.controller("formPersonaNatural", function($scope, obj, API_PERSONA, $mdDialog, msg, $rootScope, $timeout) {
    ...
    if(obj){
        ...
        API_PERSONA.PersonaNatural.get({ id: id }).$promise.then(function(success) {
            $scope.items = success;
            $scope.showErr = true;
        });
    };
    ...
    $scope.save = function() {
        $scope.errores = [];
        if ($scope.items.id) {
            API_PERSONA.PersonaNatural.update({ id: $scope.items.id }, $scope.items).$promise.then(function(r){
                msg.yesUpdate();
                $mdDialog.hide(r);
            }, function(err) {
                msg.noUpdate();
            ...
            API_PERSONA.PersonaNatural.save($scope.items).$promise.then(function(r) {
               msg.yesInsert();
               $mdDialog.hide(r);
            }, function(err) {
                msg.noInsert();
